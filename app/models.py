...
class Item(Base):
    ...
    metadata = Column(Text) # Renamed from 'metadata' to avoid conflict if using reserved word, but 'metadata' is fine in SQLA
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class WalletTransaction(Base):
    ...
    notes = Column(Text)
    metadata = Column(Text) # 'metadata' is a reserved attribute in Base, but as a column name it's usually fine if mapped correctly. To be safe, we can use 'meta_data' or keep it if it works. SQLAlchemy usually handles this.
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
