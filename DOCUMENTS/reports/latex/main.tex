...
\scalebox{0.82}{\begin{tikzpicture}[
  nd/.style={draw, circle, minimum size=0.8cm, font=\small\bfseries, fill=#1},
  edg/.style={-{Stealth[length=2mm]}, thick},
  lbl/.style={font=\scriptsize, above}
]
% ── DAG ──
\node[nd=green!25] (A) at (0,0) {A};
\node[nd=blue!15]  (B) at (2,0.8) {B};
\node[nd=blue!15]  (C) at (2,-0.8) {C};
\node[nd=orange!20](D) at (4,0) {D};
\node[nd=red!15]   (E) at (6,0) {E};
\draw[edg] (A) -- (B);
\draw[edg] (A) -- (C);
\draw[edg] (B) -- (D);
\draw[edg] (C) -- (D);
...
% ── Step annotations ──
\node[font=\scriptsize, anchor=west] at (7.2, 0.8) {Step 1: Queue = $\lbrace$A$\rbrace$ \quad (in-deg 0)};
\node[font=\scriptsize, anchor=west] at (7.2, 0.2) {Step 2: Execute A $\to$ Queue = $\lbrace$B, C$\rbrace$};
\node[font=\scriptsize, anchor=west] at (7.2,-0.4) {Step 3: Execute B, C $\to$ Queue = $\lbrace$D$\rbrace$};
\node[font=\scriptsize, anchor=west] at (7.2,-1.0) {Step 4: Execute D $\to$ Queue = $\lbrace$E$\rbrace$};
\node[font=\scriptsize, anchor=west] at (7.2,-1.6) {Step 5: Execute E $\to$ Done (5/5 = acyclic)};

% ── In-degree labels ──
\node[font=\tiny, below=1pt] at (A.south) {deg=0};
\node[font=\tiny, below=1pt] at (B.south) {deg=1};
\node[font=\tiny, below=1pt] at (C.south) {deg=1};
\node[font=\tiny, below=1pt] at (D.south) {deg=2};
\node[font=\tiny, below=1pt] at (E.south) {deg=1};
\end{tikzpicture}}
...
A light penetration test was performed against the production deployment (\texttt{auto-flowai.com}) on January~12, 2026, using an automated website vulnerability scanner (39 tests, 38-second scan). The scan targeted the Nginx~1.18.0 reverse proxy and the Flask application behind it. Table~\ref{tab:pentest} summarises the findings and the mitigations already implemented or planned.
...
Nginx 1.18.0 CVEs (CVE-2021-23017 resolver overwrite, CVE-2023-44487 HTTP/2 rapid reset, CVE-2021-3618 ALPACA, CVE-2022-41741/42 mp4 module)
  & High
  & Upgrade Nginx to $\geq$1.25.x on production VPS. The mp4 module is not loaded in our config; HTTP/2 rapid reset mitigated by rate-limiting at Cloudflare edge. \\
\midrule
...
Login interface exposed (brute-force risk)
  & Low
  & Flask-Limiter enforces 5 login attempts per minute per IP. Account lockout after 10 failed attempts (planned). \\
\midrule
